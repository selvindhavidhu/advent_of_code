...
static const char WORDS[WORDS_ARRAY_SIZE][6] = {"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
static const size_t WORDS_LENGTH[WORDS_ARRAY_SIZE] = {3, 3, 5, 4, 4, 3, 5, 5, 4};
...
static int find_first_digit(const char *line)
{
    ...
    for (int i = 0; i < line_length; ++i)
    {
        ...
        else
        {
            ...
            {
                if (len >= WORDS_LENGTH[k] && (strncmp(line + i, WORDS[k], WORDS_LENGTH[k]) == 0))
                {
                    return (k + 1);
                }
            }
